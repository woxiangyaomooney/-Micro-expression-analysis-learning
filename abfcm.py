import os
import shutil # 用于高级文件操作，比如复制、删除、移动文件等。
import glob # 用于查找符合特定规则的文件路径名。
from datetime import datetime

import numpy as np # 用于数值计算和数据处理
import pandas as pd # 用于数值计算和数据处理
import torch # PyTorch 深度学习框架，用于构建神经网络模型
from torch.autograd import Variable
from colorama import Fore # 用于给命令行添加颜色和样式
from colorama import Style
# from tqdm import tqdm
from torch.utils.tensorboard.writer import SummaryWriter # 用于 PyTorch 中的 TensorBoard 集成，用于记录训练过程中的数据和模型可视化。
...
# 用于实现多类别的 Focal Loss（焦点损失）
class MultiCEFocalLoss(torch.nn.Module):
    # 初始化方法，定义了损失函数所需的参数和属性
    def __init__(self, class_num, gamma=2, alpha=None, reduction='mean'): # 定义了使用均值损失函数
    ...
    def forward(self, predict, target):
        pt = torch.softmax(predict, dim=-1) # 通过 softmax 函数计算预测值的概率分布
        class_mask = torch.nn.functional.one_hot(target, self.class_num) # 根据真实标签创建独热编码张量
        ids = target.view(-1, 1) # 将 target 张量的形状调整为 (-1, 1)
        alpha = self.alpha[ids.data.view(-1)].view(-1, 1) # 根据标签获取相应的权重
        alpha = alpha.to(predict.device) # 将 alpha 张量移动到与 predict 张量相同的设备，确保张量在相同的设备上
